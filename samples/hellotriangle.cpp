...
std::pair<SDL_Window*, void*> createSDLwindow() {
    ...
    SDL_Window* win = SDL_CreateWindow("Hello World!", -100, 100, 800, 800, windowFlags);
    if (win == nullptr) {
...
struct IconScene {
    ...
    IconScene(Engine* engine, Renderer* renderer, uint32_t w, uint32_t h, Option option)
        ...
        if (option.useSkybox) {
            skybox = Skybox::Builder().color({0.5, 0.5, 0.5, 1.0}).build(*engine);
        ...
        view->setPostProcessingEnabled(option.postProcessing);
...
int main() {
    ...
    IconScene iconScene {engine, renderer, iconWidth, iconHeight, {.useSkybox=true} };

    std::vector<Texture*> intermediate;
    std::vector<std::vector<Texture*>> targets1;
    std::vector<std::vector<Texture*>> targets0;
    for (size_t r = 0; r < nrow; r++) {
        std::vector<Texture*> row0;
        std::vector<Texture*> row1;
        for (size_t c = 0; c < ncol; c++) {
            auto colorTexture0 = Texture::Builder()
                    .width(iconWidth).height(iconHeight).levels(1)
                    .usage(Texture::Usage::COLOR_ATTACHMENT | Texture::Usage::SAMPLEABLE)
                    .format(Texture::InternalFormat::RGBA8).build(*engine);
            row0.push_back(colorTexture0);

            auto colorTexture1 = Texture::Builder()
                    .width(iconWidth).height(iconHeight).levels(1)
                    .usage(Texture::Usage::COLOR_ATTACHMENT | Texture::Usage::SAMPLEABLE)
                    .format(Texture::InternalFormat::RGBA8).build(*engine);
            row1.push_back(colorTexture1);

        }
        targets0.push_back(row0);
        targets1.push_back(row1);
    }

    MainScene scene(engine, renderer, w, h, nrow, ncol);

    renderer->setClearOptions({.clear = false});
    float3 const a {.8, .3, .3};
    float3 const b {.3, .8, .3};
    ...
    IconScene swapChainScene{engine, renderer, w, h, {.border = 1.0f, .postProcessing = false}};
    ...
    while (!closeWindow && ++count) {
        ...
        float3 color = (r + c) % 2 == 0 ? a : b;
        auto rt0 = targets0[r][c];
        auto rt1 = targets1[r][c];
        iconScene.render(matTexture.get(), rt0, color);
//        engine->flushAndWait();
        iconScene.render(rt0, rt1, float3(1, 1, 1) * .4);
//        engine->flushAndWait();
        scene.render(rt1, mainRT, r, c);
//        engine->flushAndWait();
//        engine->flush();
