...
FMaterial::FMaterial(FEngine& engine, const Builder& builder, MaterialDefinition const& definition)
    ...
    mDefinition.acquirePrograms(engine, mSpecializationConstants, mIsDefaultMaterial);
...
void FMaterial::terminate(FEngine& engine) {
    ...
    mDefinition.releasePrograms(engine, mSpecializationConstants, mIsDefaultMaterial);
...
void FMaterial::prepareProgram(Variant const variant,
    ...
    mEngine.getMaterialCache().prepareProgram(mEngine, mDefinition,
            getProgramSpecialization(variant), priorityQueue);
}

[[nodiscard]]
backend::Handle<backend::HwProgram> FMaterial::getProgram(Variant const variant) const noexcept {
#if FILAMENT_ENABLE_MATDBG
    ...
    if (isSharedVariant(variant)) {
        FMaterial const* defaultMaterial = mEngine.getDefaultMaterial();
        FILAMENT_CHECK_PRECONDITION(defaultMaterial);
        return defaultMaterial->getProgram(variant);
    }
...
bool FMaterial::setConstant(uint32_t id, T value) noexcept {
    ...
    mDefinition.acquirePrograms(mEngine, mSpecializationConstants, mIsDefaultMaterial);

    // Then release old ones.
    mDefinition.releasePrograms(mEngine, prevSpecializationConstants, mIsDefaultMaterial);
