...
void MaterialDefinition::acquirePrograms(FEngine& engine,
    ...
    // cache which is initialized later. This is done so that we can indiscriminately release all
    // programs in releasePrograms() without having to keep track of which programs we've actually
    // compiled.
    for (auto variant: getVariants()) {
        if (UTILS_LIKELY(hasVariant(variant, shaderModel, isStereoSupported))) {
            ...
            materialCache.acquireProgram(specialization);
        }
    }

    // Now do depth variants. We only do these if this is a default material or if the material has
    ...
    if (UTILS_UNLIKELY(isDefaultMaterial || hasCustomDepthShader)) {
        if (UTILS_UNLIKELY(isDefaultMaterial && !engine.getDriverApi().isWorkaroundNeeded(
            ...
            for (auto variant: getDepthVariants()) {
                if (UTILS_LIKELY(hasVariant(variant, shaderModel, isStereoSupported))) {
                    specialization.variant = variant;
                    materialCache.acquireAndPrepareProgram(engine, *this, specialization,
